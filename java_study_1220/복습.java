...
/*
 * 1. 모든 Java의 클래스는 Object를 상속 받고 있음
 * 2. 기본생성자
*/
class B extends Object{
	...
	public B(int x) { // 파라미터 값이 존재한 생성자
		this.x = x; // this를 이용해서 파라미터 값을 피드변수 'x'에 대입
	}
...
// extends : 상속
// Java는 다중 상속을 지원 X => interface
// implements : 구현하다('A'클래스는 'C'를 구현하다.)
class A extends B implements C{
	// A 클래스는 '자식 클래스' B클래스는 '부모 클래스'
	/*
	 * A클래스는 B클래스에 선언된 메소드와 필드변수를 사용할 수 있음
	 * 단, 접근지정자가 public 혹은 protected로 되어 있어야함.  
	*/
	@Override
	public void showMessage() { //부모 클래스 메소드 '재정의'(오버라이딩)
